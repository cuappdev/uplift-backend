...
def delete_migrations():
    try:
        ...
        os.system(
            "export PGPASSWORD={}; psql --user={} --host={} -p {} {} ".format(
...
def init_data():
    ...
    _, juan_garcia_post = pd.create_post(
        biography=(
            "My name is Juan Garcia and I'm a junior studying Operations Research and Information Engineering "
    ...
    _, juan_garcia_cardio = rd.create_routine(
        ...
        steps=(
            "If you're going for distance, start off at a distance that you are comfortable with(i.e. 1 mile) "
    ...
    _, juan_garcia_strength = rd.create_routine(
        ...
        post_id=juan_garcia_post.id,
        steps=(
            "Know what your one rep max is for compound movements which include Bench Press, Squats, and Deadlifts "
    ...
    _, juan_garcia_mindfulness = rd.create_routine(
        ...
        post_id=juan_garcia_post.id,
        steps=(
            "I'd recommend either reading a book for 10 min a day or going on a walk for 10-15 without music "
            "playing."
        ),
    ...
    _, madeline_ugarte_post = pd.create_post(
        biography=(
            "I'm a National Academy of Sports Medicine Certified Personal Trainer and Performance Enhancement "
            "Specialist (NASM-CPT, PES), and I'm a certified instructor in Spinning, SpinPower, TRX, Power HIIT "
            "and Shockwave. Other than that, I'm a proud Chicago native and a senior at Cornell. Can't wait to "
            "see you in a class!"
        ),
    ...
    _, madeline_ugarte_cardio = rd.create_routine(
        ...
        steps=(
            "You're already probably getting your steps in without even realizing it (you can thank all the Cornell "
            "hills for that!). So to step it up a notch I'd recommend a group fitness class like Spinning for "
            "beginners so they can increase their cardio performance with some guidance!"
        ),
    ...
    _, madeline_ugarte_strength = rd.create_routine(
        ...
        post_id=madeline_ugarte_post.id,
        steps=(
            "Start small, stay consistent and pick something you actually like! My advice would be to pick a skill "
            "that you'd like to improve and incorporate it consistently at the end of your workouts or throughout the "
            "day. When I first started out I really wanted to get better at pushups so I always ended my workouts with "
            "10 pushups, now it's part of my warm up instead!"
        ),
    ...
    _, madeline_ugarte_mindfulness = rd.create_routine(
        ...
        post_id=madeline_ugarte_post.id,
        steps=(
            "Give yourself 30 minutes a day to do something that makes you happy. I wake up earlier than I need to "
            "every morning for an oat milk latte and 30 minutes to lightly stretch and go through my Instagram feed"
    ...
    _, madeline_ugarte_sm = smd.create_social_media(
        instagram="https://www.instagram.com/trainwithmadeline/", post_id=madeline_ugarte_post.id
    )
    ...
    _, starks_twins_post = pd.create_post(
        biography=(
            "We're twin powerbuilders: people who lift to get stronger and look better. As students, we're "
            "computational biology majors who intend to attend graduate school in bioinformatics, computational "
    ...
    _, starks_twins_cardio = rd.create_routine(
        ...
        steps=(
            "Walk to class everyday. Unless it's sub-zero outside, walking to class on a campus like Cornell will "
    ...
    _, starks_twins_strength = rd.create_routine(
        ...
        post_id=starks_twins_post.id,
        steps=(
            "Go to the gym, and go consistently. Even if you plan to go twice per week, it is important to establish "
    ...
    _, starks_twins_mindfulness = rd.create_routine(
        ...
        post_id=starks_twins_post.id,
        steps=(
            "Take time for yourself everyday. It is important to relax your mind and brain and do something you "
    ...
    _, starks_twins_sm = smd.create_social_media(
        instagram="https://www.instagram.com/starkstwins/", post_id=starks_twins_post.id
    )
    ...
    _, cleo_kyriakides_post = pd.create_post(
        biography=(
            "I'm a long distance runner who got hooked onto CrossFit along the way! I love feeling strong and "
            "sharing my knowledge to help people reach their goals. I'm also the 2018-19 Fitness Club prez and a "
            "member of Track Club!"
        ),
        college="Engineering",
    ...
    _, cleo_kyriakides_cardio = rd.create_routine(
        ...
        steps=(
            "Start out with small, achievable goals. Start by running up to a mile at a time every time you run, and "
    ...
    _, cleo_kyriakides_strength = rd.create_routine(
        ...
        post_id=cleo_kyriakides_post.id,
        steps=(
            "First of all, form is EVERYTHING. Work with someone who can spot you for the movements as you start. "
            "Muscle fatigue each time is not desired, so start by doing small rep/low weight circuits of simple "
            "movements (back squats, presses, deadlifts) 3x a week, increasing weight 5-10lbs each week."
        ),
    ...
    _, cleo_kyriakides_mindfulness = rd.create_routine(
        ...
        post_id=cleo_kyriakides_post.id,
        steps=(
            "I am an avid knitter and I do yoga regularly. Find something that relaxes you that isn't physically "
    ...
    _, cleo_kyriakides_sm = smd.create_social_media(
        instagram="https://instagram.com/fitnessbycleo", post_id=cleo_kyriakides_post.id
    )
    ...
    _, clarie_ng_post = pd.create_post(
        biography=(
            "A proponent of every body is beautiful. I like to tailor workouts for myself because each person has "
            "different gene, muscle insertions, and body proportions. There's no 'end' to a fitness journey in "
            "my opinion but a lifestyle to celebrate the amazing capabilities of our body. I believe our body was "
            "made to move and feel incredible. I hope to set people on the right path towards loving themselves "
            "and exercise."
        ),
        college="CALS",
    ...
    _, clarie_ng_cardio = rd.create_routine(
        ...
        steps=(
            "10,000 Steps a day or incorporate walking from place to places. I rather someone be active the entire "
            "day than run for 30 minutes and sit on the couch for the rest of the day. Cardio should not be a chore "
            "but assimilated into your life. (unless you have some speed or endurance goal or have a time crunch "
            "during exam season)"
        ),
    ...
    _, clarie_ng_strength = rd.create_routine(
        ...
        post_id=clarie_ng_post.id,
        steps=(
            "PROPER form is king! Never lift something heavier for the sake of ego. You can make better muscle gains"
            " by going lighter but doing the full range of motion. Body weights are fantastic and should be the goal "
            "of everyone: imagine being able to bench heavy and yet can't even do a push up. That's lame. Being able "
            "to carry your own weight is a sign of balance!"
        ),
    ...
    _, clarie_ng_mindfulness = rd.create_routine(
        ...
        post_id=clarie_ng_post.id,
        steps=(
            "Do something for yourself. Chase your own best version of you and not how you think other people will "
    ...
    _, clarie_ng_sm = smd.create_social_media(
        instagram="https://www.instagram.com/thenvironmentalistbodybuilder/", post_id=clarie_ng_post.id
    )
    ...
    _, mark_rittiboon_post = pd.create_post(
        biography=(
            "I used to be a competitive strength athlete participating in powerlifting and strongman competitions."
            " However, I've transitioned into combat sports ~ recently completing a fighting camp in Phuket "
            "Thailand!"
        ),
    ...
    _, mark_rittiboon_cardio = rd.create_routine(
        ...
        steps=(
            "I don't enjoy running and Ithaca is really cold so I don't do it haha... I find swimming to be really"
    ...
    _, mark_rittiboon_strength = rd.create_routine(
        ...
        post_id=mark_rittiboon_post.id,
        steps=(
            "I think everyone looking to build strength should focus on compound lifts (Squats, Deadlifts, Bench, etc)"
    ...
    _, mark_rittiboon_mindfulness = rd.create_routine(
        ...
        post_id=mark_rittiboon_post.id,
        steps=(
            "I love to cook! Nothing like Spotify's 'cooking music' playlist and eggs in the morning :) "
    ...
    _, mark_rittiboon_sm = smd.create_social_media(
        facebook="https://www.facebook.com/mark.rittiboon", post_id=mark_rittiboon_post.id
    )
